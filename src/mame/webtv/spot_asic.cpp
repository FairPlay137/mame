...
void spot_asic_device::device_reset()
{
	...
	dev_idcntl = 0x00;
	dev_id_state = DS2401_STATE_IDLE;
	dev_id_bit = 0x0;
	dev_id_bitidx = 0x0;
...
// HACK: The WebTV software does not read back the data correctly due to buggy DS2401 delay logic, so we're breaking out of the normal code for the device, instead using our own as a workaround. If a better solution is found, this hack can be removed.
uint32_t spot_asic_device::reg_4008_r()
{
	dev_id_bit = 0x0;

	if (dev_id_state == DS2401_STATE_PRESENCE)
	{
		dev_id_bit = 0x0; // We're present.
		dev_id_state = DS2401_STATE_COMMAND; // This normally would stay in presence mode for 480us then command, but we immediatly go into command mode.
		dev_id_bitidx = 0x0;
	}
	else if (dev_id_state == DS2401_STATE_READROM_PULSEEND)
	{
		dev_id_state = DS2401_STATE_READROM_BIT;
	}
	else if (dev_id_state == DS2401_STATE_READROM_BIT)
	{
		dev_id_state = DS2401_STATE_READROM; // Go back into the read ROM pulse state
		...
		if (dev_id_bitidx == 64)
		{
			// We've read the entire chip. Go back into idle.
			dev_id_state = DS2401_STATE_IDLE;
			dev_id_bitidx = 0x0;
		}
	}
...
void spot_asic_device::reg_4008_w(uint32_t data)
{
	...
	if (dev_idcntl & 0x2)
	{
		...
		{
			case DS2401_STATE_RESET: // End reset low pulse to go into prescense mode. Chip should read low to indicate presence.
				dev_id_state = DS2401_STATE_PRESENCE; // This pulse normally lasts 480us before going into command mode.
				break;

			case DS2401_STATE_COMMAND: // Ended a command bit pulse. Increment bit index. We always assume a read from ROM command after we get 8 bits.
				dev_id_bitidx++;

				if (dev_id_bitidx == 8)
				{
					dev_id_state = DS2401_STATE_READROM; // Now we can read back the chip. Software reads it as two 32-bit integers.
					dev_id_bitidx = 0;
				}
				break;

			case DS2401_STATE_READROM_PULSESTART:
				dev_id_state = DS2401_STATE_READROM_PULSEEND;
		}
	}
	else
	{
		...
		{
			case DS2401_STATE_IDLE: // When idle, we can drive the chip low for reset
				dev_id_state = DS2401_STATE_RESET; // We'd normally leave this for 480us to go into presence mode.
				break;

			case DS2401_STATE_READROM:
				dev_id_state = DS2401_STATE_READROM_PULSESTART;
				break;
		}
	}
}
...
//
// The SPOT programming doc calls this as an NVCNTL register but this is used as an I2C register, mirroring its purpose in SOLO.
//
